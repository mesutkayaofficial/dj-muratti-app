...
st.markdown("""
    ...
    .stButton>button {
        width: 90%; margin-left: 5%; border-radius: 100px;
        background: linear-gradient(90deg, #00f5ff, #1ed760);
        color: black; font-weight: 800; border: none; height: 3.8em;
...
# Senin HTML Tasarımın + Dinamik Veri & Tıklanabilirlik
def get_dashboard_html(total_views="8.4M", video_rows=""):
    html_template = f"""
    <!DOCTYPE html>
    <html lang="tr">
    <head>
    ...
    <style>
      ...
      .pulse-card {{ background: var(--surface); border: 1px solid rgba(255,255,255,0.06); padding: 30px; border-radius: 32px; text-align: center; margin-bottom: 25px; }}
      .pulse-value {{ font-size: 42px; font-weight: 800; color: var(--cyan); margin: 10px 0; }}
      
      /* Tıklanabilir Video Kartı Tasarımı */
      .video-item {{ 
      ...
      .video-item:active {{ transform: scale(0.96); background: rgba(255,255,255,0.08); }}
      
      .video-rank {{ font-weight: 800; color: var(--muted); font-size: 12px; margin-right: 15px; }}
      .video-views {{ font-weight: 700; color: var(--green); font-size: 16px; }}
      .video-user {{ font-weight: 700; font-size: 14px; color: var(--text); }}
    ...
    <div class="container">
      ...
      <div class="pulse-card">
        ...
        <div style="font-size: 11px; color: var(--green); font-weight: 600; letter-spacing: 1px;">LIVE SYSTEM ACTIVE</div>
      </div>
      <div style="font-family: 'Orbitron'; font-size: 12px; color: var(--cyan); margin-bottom: 15px; letter-spacing: 2px;">🔝 TOP 50 PROOFS (CLICK TO WATCH)</div>
      <div class="video-list">{video_rows}</div>
    </div>
...
if st.button("🚀 CANLI ANALİZİ BAŞLAT"):
    with st.spinner("Sistem Senkronize Ediliyor..."):
        ...
        try:
            ...
            # Tıklanabilir Video Satırları Oluşturma
            video_html = ""
            ...
            for i, v in enumerate(sorted_vids[:50], 1):
                ...
                # 'onclick' ekleyerek kartın tamamını tıklanabilir yaptık
                video_html += f"""
                <div class="video-item" onclick="window.open('{v_url}', '_blank')">
                    <div style="display:flex; align-items:center;">
                        ...
                        <div>
                            ...
                            <div style="font-size:10px; color:var(--muted);">TAP TO VIEW PROOF</div>
                        </div>
                    </div>
                    <div class="video-views">{v.get('play_count', 0):,}</div>
                </div>
                """
            st.components.v1.html(get_dashboard_html(f"{total_views_sum:,}", video_html), height=1500, scrolling=True)
...
st.caption(f"DJ MURATTI HQ | {time.strftime('%H:%M:%S')}")
